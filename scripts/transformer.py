...
########################################################################
class CharacterInvertTransformer:
    def __init__(self):
        pass
    ...
    def text2features(self, text):
        ...
        array.reverse()
        g_1_1 = getNgram(array, (-4, -4), mode='letter', invert=True)
        g_1_2 = getNgram(array, (-3, -3), mode='letter', invert=True)
        g_1_3 = getNgram(array, (-2, -2), mode='letter', invert=True)
        g_1_4 = getNgram(array, (-1, -1), mode='letter', invert=True)
        g_1_5 = getNgram(array, (0, 0), mode='letter', invert=True)
        g_1_6 = getNgram(array, (1, 1), mode='letter', invert=True)
        g_1_7 = getNgram(array, (2, 2), mode='letter', invert=True)
        g_1_8 = getNgram(array, (3, 3), mode='letter', invert=True)
        g_1_9 = getNgram(array, (4, 4), mode='letter', invert=True)
        g_2_1 = getNgram(array, (-1, 0), mode='letter', invert=True)
        g_2_2 = getNgram(array, (0, 1), mode='letter', invert=True)
        g_3_1 = getNgram(array, (-2, 0), mode='letter', invert=True)
        g_3_2 = getNgram(array, (-1, 1), mode='letter', invert=True)
        g_3_3 = getNgram(array, (0, 2), mode='letter', invert=True)
        g_4_1 = getNgram(array, (-3, 0), mode='letter', invert=True)
        g_4_2 = getNgram(array, (-2, 1), mode='letter', invert=True)
        g_4_3 = getNgram(array, (-1, 2), mode='letter', invert=True)
        g_4_4 = getNgram(array, (0, 3), mode='letter', invert=True)
        g_5_1 = getNgram(array, (-4, 0), mode='letter', invert=True)
        g_5_2 = getNgram(array, (-3, 1), mode='letter', invert=True)
        g_5_3 = getNgram(array, (-2, 2), mode='letter', invert=True)
        g_5_4 = getNgram(array, (-1, 3), mode='letter', invert=True)
        g_5_5 = getNgram(array, (0, 4), mode='letter', invert=True)
    ...
    def invert_ann(self, ann, text_length):
        """annを受けて逆向きにして返す。
        >>> invert_ann({'entity': (5, 10)}, text_lenth=100)
        >>> {'ytitne': (90, 95)}
        """
        return {k[::-1]: (text_length - v[1], text_length - v[0]) for k, v in ann.items()}
    ...
    def text_anns2labels(self, text, anns):
        ...
        labels = []
        ann_ix = 0
        invert_anns = [self.invert_ann(ann, len(text)) for ann in anns]
        ann_values = [v for ann in invert_anns for k, v in ann.items()]
        ann_values.reverse()
    ...
    def text_labels2anns(self, text, labels):
        ...
        entity = ''
        start = 0
        for i, (c, label) in enumerate(zip(text[::-1], labels)):
            if label == 'O':
                pass
            elif label == 'S':
            ...
            # EとBが逆転しているので注意
                ...
                if not entity == ' ':
                #entity += c
                    anns.append({entity: (start, i)})
                entity = ''
        return [self.invert_ann(ann, len(text)) for ann in anns]
...
def getNgram(array, n_range=(-2, 2), mode='letter', invert=False):
    ...
    ret_array = []
    if invert:
        pagging_array = ['<E>'] * max(0, -1 * n_range[0]) + array + ['<B>'] * max(n_range[1], 0)
    else:
